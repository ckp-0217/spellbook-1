...
WITH element_txs AS (
        ...
        FROM `element_ex_bnb`.`OrdersFeature_evt_ERC721SellOrderFilled` ee
        ...
        , ee.evt_tx_hash AS tx_hash
        , ee.evt_block_number AS block_number
        FROM `element_ex_bnb`.`OrdersFeature_evt_ERC721BuyOrderFilled` ee
        ...
        , ee.evt_tx_hash AS tx_hash
        , ee.evt_block_number AS block_number
        FROM `element_ex_bnb`.`OrdersFeature_evt_ERC1155SellOrderFilled` ee
        ...
        , ee.evt_tx_hash AS tx_hash
        , ee.evt_block_number AS block_number
        FROM `element_ex_bnb`.`OrdersFeature_evt_ERC1155BuyOrderFilled` ee
...
LEFT JOIN `nft`.`aggregators` agg ON alet.buyer=agg.contract_address AND agg.blockchain='bnb'
LEFT JOIN `tokens`.`erc20` bnb_bep20_tokens ON bnb_bep20_tokens.contract_address=alet.currency_contract AND bnb_bep20_tokens.blockchain='bnb'
LEFT JOIN `tokens`.`nft` bnb_nft_tokens ON bnb_nft_tokens.contract_address=alet.currency_contract AND bnb_nft_tokens.blockchain='bnb'
...
LEFT JOIN `bnb`.`transactions` bt ON bt.hash=alet.tx_hash
