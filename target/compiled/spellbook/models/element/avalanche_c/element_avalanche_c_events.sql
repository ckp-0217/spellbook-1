...
WITH element_txs AS (
        ...
        FROM `element_ex_avalanche_c`.`OrdersFeature_evt_ERC721SellOrderFilled` ee
        ...
        , ee.evt_tx_hash AS tx_hash
        , ee.evt_block_number AS block_number
        FROM `element_ex_avalanche_c`.`OrdersFeature_evt_ERC721BuyOrderFilled` ee
        ...
        , ee.evt_tx_hash AS tx_hash
        , ee.evt_block_number AS block_number
        FROM `element_ex_avalanche_c`.`OrdersFeature_evt_ERC1155SellOrderFilled` ee
        ...
        , ee.evt_tx_hash AS tx_hash
        , ee.evt_block_number AS block_number
        FROM `element_ex_avalanche_c`.`OrdersFeature_evt_ERC1155BuyOrderFilled` ee
...
LEFT JOIN `nft`.`aggregators` agg ON alet.buyer=agg.contract_address AND agg.blockchain='avalanche_c'
LEFT JOIN `tokens`.`erc20` ava_erc20_tokens ON ava_erc20_tokens.contract_address=alet.currency_contract AND ava_erc20_tokens.blockchain='avalanche_c'
LEFT JOIN `tokens`.`nft` ava_nft_tokens ON ava_nft_tokens.contract_address=alet.currency_contract AND ava_nft_tokens.blockchain='avalanche_c'
...
LEFT JOIN `avalanche_c`.`transactions` at ON at.hash=alet.tx_hash
