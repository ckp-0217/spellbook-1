...
with v1 as (
    select
        ...
        '1' as orderType
    from `liquidifty_bnb`.`MarketplaceV1_evt_Buy`
...
v2 as (
    select
        ...
        price as amount_raw,
        case
            when currency = '0x0000000000000000000000000000000000000000' then '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c'
            else currency
    ...
    from (
        select *
        from `liquidifty_bnb`.`MarketplaceV2_evt_Buy`
        
        union all
        select *
        from `liquidifty_bnb`.`MarketplaceV2_1_evt_Buy`
        
        union all
        select *
        from `liquidifty_bnb`.`MarketplaceV2_5_evt_Buy`
...
stack as (
    select
        ...
        '1' as orderType
    from `liquidifty_bnb`.`MultipleSell_evt_Buy`
...
v3 as (
    select
        ...
        case
            when get_json_object(currency, '$.assetType') = '0' then '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c'
    ...
    from (
        ...
        from (
            ...
            from (
                ...
                from `liquidifty_bnb`.`MarketplaceV3_call_buy`
                
                union all
                select *, explode(orders) as order
                from `liquidifty_bnb`.`MarketplaceV3_deprecated_call_buy`
                )
            where call_success
        )
    )
...
left join `tokens_bnb`.`bep20` bep20
    on bep20.contract_address = buys.currency_contract
left join `tokens_bnb`.`nft` nft_tokens
...
inner join `bnb`.`transactions` transactions
