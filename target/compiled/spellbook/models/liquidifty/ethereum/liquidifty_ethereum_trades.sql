...
with v2 as (
    select
        ...
        price as amount_raw,
        case
            when currency = '0x0000000000000000000000000000000000000000' then '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
            else currency
        ...
        '1' as orderType
    from `liquidifty_ethereum`.`MarketplaceV2_5_evt_Buy`
...
stack as (
    select
        ...
        '1' as orderType
    from `liquidifty_ethereum`.`PoolSell_evt_Buy`
...
v3 as (
    select
        ...
        case
            when get_json_object(currency, '$.assetType') = '0' then '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
    ...
    from (
        ...
        from (
            ...
            from (
                ...
                from `liquidifty_ethereum`.`MarketplaceV3_call_buy`
                
                union all
                select *, explode(orders) as order
                from `liquidifty_ethereum`.`MarketplaceV3_deprecated_call_buy`
...
left join `tokens_ethereum`.`nft` nft_tokens
...
inner join `ethereum`.`transactions` transactions
