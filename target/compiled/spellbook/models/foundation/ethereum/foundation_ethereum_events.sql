...
WITH all_foundation_trades AS (
    ...
    , f.evt_index
    FROM `foundation_ethereum`.`market_evt_ReserveAuctionFinalized` f
    LEFT JOIN `foundation_ethereum`.`market_evt_ReserveAuctionCreated` c ON c.auctionId=f.auctionId AND c.evt_block_time<=f.evt_block_time
    ...
    , evt_index
    FROM `foundation_ethereum`.`market_evt_BuyPriceAccepted` f
    ...
    , creatorRev/POWER(10, 18) AS royalty_fee_amount
    , evt_index
    FROM `foundation_ethereum`.`market_evt_OfferAccepted` f
    ...
    , evt_index
    FROM `foundation_ethereum`.`market_evt_PrivateSaleFinalized` f
...
LEFT JOIN `tokens_ethereum`.`nft` nft ON t.nft_contract_address=nft.contract_address
LEFT JOIN `foundation_ethereum`.`market_evt_SellerReferralPaid` sellref ON sellref.evt_block_time = t.block_time AND sellref.evt_tx_hash = t.tx_hash
LEFT JOIN `erc721_ethereum`.`evt_transfer` nft_t ON nft_t.evt_block_time=t.block_time
...
LEFT JOIN `ethereum`.`transactions` et ON et.block_time=t.block_time
...
LEFT JOIN `prices`.`usd` pu ON pu.minute=date_trunc('minute', t.block_time)
    AND pu.blockchain='ethereum'
    AND pu.contract_address=t.currency_contract
    
LEFT JOIN `ethereum`.`traces` ett ON ett.block_time=t.block_time
