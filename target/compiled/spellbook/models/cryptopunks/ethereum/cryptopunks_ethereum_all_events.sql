...
(
    select  evt_block_time
            ...
            , evt_block_number
            , evt_tx_hash
    from `cryptopunks_ethereum`.`punk_bid_events`
    ...
    select evt_block_time
            ...
            , evt_block_number
            , evt_tx_hash
    from `rarible_v1_ethereum`.`ERC721Sale_v2_evt_Buy`
    ...
    where evt_tx_hash not in (select distinct tx_hash from `cryptopunks_ethereum`.`trades` )
) a 
