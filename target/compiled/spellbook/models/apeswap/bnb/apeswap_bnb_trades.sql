...
WITH apeswap_dex AS (
    SELECT  t.evt_block_time AS block_time,
            ...
            '' AS trace_address,
            t.evt_index
    FROM `apeswap_bnb`.`ApePair_evt_Swap` t
    INNER JOIN `apeswap_bnb`.`ApeFactory_evt_PairCreated` f
        ON f.pair = t.contract_address
    
    
    WHERE t.evt_block_time >= '2021-02-13'
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2021-02-13'
