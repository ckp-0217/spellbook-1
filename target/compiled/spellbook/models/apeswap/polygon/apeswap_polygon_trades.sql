...
WITH apeswap_dex AS (
    SELECT  t.evt_block_time AS block_time,
            ...
            '' AS trace_address,
            t.evt_index
    FROM `apeswap_polygon`.`ApePair_evt_Swap` t
    INNER JOIN `apeswap_polygon`.`ApeFactory_evt_PairCreated` p
        ON t.contract_address = p.pair
    
    
    WHERE t.evt_block_time >= '2021-03-15'
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2021-03-15'
