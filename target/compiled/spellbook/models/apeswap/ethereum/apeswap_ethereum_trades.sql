...
with dexs as (
    ...
    select
        ...
        t.evt_index
    from
        `apeswap_ethereum`.`ApePair_evt_Swap` t
        inner join `apeswap_ethereum`.`ApeFactory_evt_PairCreated` f
            on f.pair = t.contract_address
    
    
    where t.evt_block_time >= '2020-09-04'
...
left join `prices`.`usd` p_bought 
    ...
    and p_bought.minute >= '2020-09-04'
