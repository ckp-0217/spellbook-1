...
FROM `balancer_v2_arbitrum`.`Vault_evt_PoolRegistered` registered
INNER JOIN `balancer_v2_arbitrum`.`WeightedPoolFactory_call_create` call_create
...
SELECT
    ...
    weights.normalized_weight / POWER(10, 18) AS normalized_weight
FROM `balancer_v2_arbitrum`.`Vault_evt_PoolRegistered` registered
INNER JOIN `balancer_v2_arbitrum`.`WeightedPool2TokensFactory_call_create` call_create
    ON call_create.output_0 = SUBSTRING(registered.poolId, 0, 42)
    LATERAL VIEW posexplode(call_create.tokens) tokens AS pos, token_address
    LATERAL VIEW posexplode(call_create.weights) weights AS pos, normalized_weight
WHERE tokens.pos = weights.pos
...
SELECT
    ...
    weights.normalized_weight / POWER(10, 18) AS normalized_weight
FROM `balancer_v2_arbitrum`.`Vault_evt_PoolRegistered` registered
INNER JOIN `balancer_v2_arbitrum`.`WeightedPoolV2Factory_call_create` call_create
    ON call_create.output_0 = SUBSTRING(registered.poolId, 0, 42)
    LATERAL VIEW posexplode(call_create.tokens) tokens AS pos, token_address
