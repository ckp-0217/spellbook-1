...
WITH pool_labels AS (
    SELECT
        address AS pool_id,
        name AS pool_symbol
    FROM `labels`.`balancer_v2_pools_optimism`
    ),
...
zipped_balance_changes AS (
        ...
        FROM `balancer_v2_optimism`.`Vault_evt_PoolBalanceChanged`
    ),
    ...
    managed_changes AS (
        ...
        FROM `balancer_v2_optimism`.`Vault_evt_PoolBalanceManaged`
    ),
   ...
   cumulative_usd_balance AS (
    ...
    pool_liquidity_estimates AS (
        ...
        LEFT JOIN `balancer_v2_optimism`.`pools_tokens_weights` w ON b.pool_id = w.pool_id
