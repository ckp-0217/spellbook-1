...
FROM `balancer_v2_polygon`.`Vault_evt_PoolRegistered` registered
INNER JOIN `balancer_v2_polygon`.`WeightedPoolFactory_call_create` call_create
...
SELECT
    ...
    weights.normalized_weight / POWER(10, 18) AS normalized_weight
FROM `balancer_v2_polygon`.`Vault_evt_PoolRegistered` registered
INNER JOIN `balancer_v2_polygon`.`WeightedPool2TokensFactory_call_create` call_create
...
SELECT
    ...
    weights.normalized_weight / POWER(10, 18) AS normalized_weight
FROM `balancer_v2_polygon`.`Vault_evt_PoolRegistered` registered
INNER JOIN `balancer_v2_polygon`.`WeightedPoolV2Factory_call_create` call_create
    ON call_create.output_0 = SUBSTRING(registered.poolId, 0, 42)
    LATERAL VIEW posexplode(call_create.tokens) tokens AS pos, token_address
