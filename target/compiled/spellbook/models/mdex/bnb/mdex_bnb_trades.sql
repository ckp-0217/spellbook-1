...
WITH mdex_dex AS (
    SELECT  t.evt_block_time                                             AS block_time,
            ...
            ''                                                           AS trace_address,
            t.evt_index
    FROM `mdex_bnb`.`MdexPair_evt_Swap` t
    INNER JOIN `mdex_bnb`.`MdexFactory_evt_PairCreated` f
        ON f.pair = t.contract_address
    
    
    WHERE t.evt_block_time >= '2021-04-05'
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2021-04-05'
