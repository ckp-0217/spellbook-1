...
with minute as -- This CTE generates a series of minute values
         (
            ...
            SELECT explode(sequence(TIMESTAMP '2021-12-22 06:07', CURRENT_TIMESTAMP, INTERVAL 1 minute)) AS minute
         ...
         (
             with pool_amount_raw as (
                 SELECT date_trunc('minute', call_block_time) AS block_minute,
                        output_0                              as amount,
                        _0                                    as token
                 FROM `gmx_avalanche_c`.`Vault_call_poolAmounts`
                 where call_success = true
                 
                 
                 AND call_block_time >= '2021-12-22 06:07'
         ...
         (
             with rsv_amount_raw as (
                 select date_trunc('minute', call_block_time) as block_minute,
                        output_0                              as amount,
                        _0                                    as token
                 FROM `gmx_avalanche_c`.`Vault_call_reservedAmounts`
                 where call_success = true
         ...
         (
             with grt_usd_raw as (
                 select date_trunc('minute', call_block_time) as block_minute,
                        output_0                              as amount,
                        _0                                    as token
                 FROM `gmx_avalanche_c`.`Vault_call_guaranteedUsd`
                 where call_success = true
         ...
         (
             with max_price_raw as (
                 select date_trunc('minute', call_block_time) as block_minute,
                        output_0                              as price,
                        _token                                as token
                 FROM `gmx_avalanche_c`.`Vault_call_getMaxPrice`
                 where call_success = true
         ...
         (
             with min_price_raw as (
                 select date_trunc('minute', call_block_time) as block_minute,
                        output_0                              as price,
                        _token                                as token
                 FROM `gmx_avalanche_c`.`Vault_call_getMinPrice`
                 where call_success = true
         ...
         (
             with global_short_average_price_raw as (
                 select date_trunc('minute', call_block_time) as block_minute,
                        output_0                              as price,
                        _0                                    as token
                 FROM `gmx_avalanche_c`.`Vault_call_globalShortAveragePrices`
                 where call_success = true
         ...
         (
             with global_short_size_raw as (
                 select date_trunc('minute', call_block_time) as block_minute,
                        output_0                              as amount,
                        _0                                    as token
                 FROM `gmx_avalanche_c`.`Vault_call_globalShortSizes`
                 where call_success = true
