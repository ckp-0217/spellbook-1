...
(
    ...
    SELECT
        ...
        ,t.evt_index
    FROM
        `arbswap_arbitrum`.`SwapPair_evt_Swap` t
    INNER JOIN `arbswap_arbitrum`.`SwapFactory_evt_PairCreated` f
        ON f.pair = t.contract_address
    
    
    WHERE t.evt_block_time >= '2022-05-29'
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2022-05-29'
