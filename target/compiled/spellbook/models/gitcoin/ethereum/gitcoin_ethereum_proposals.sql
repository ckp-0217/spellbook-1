...
FROM  `gitcoin_ethereum`.`GovernorAlpha_evt_ProposalCreated` pcr
LEFT JOIN cte_sum_votes csv ON csv.proposalId = pcr.id
LEFT JOIN `gitcoin_ethereum`.`GovernorAlpha_evt_ProposalCanceled` pca ON pca.id = pcr.id
LEFT JOIN `gitcoin_ethereum`.`GovernorAlpha_evt_ProposalExecuted` pex ON pex.id = pcr.id
LEFT JOIN `gitcoin_ethereum`.`GovernorAlpha_evt_ProposalQueued` pqu ON pex.id = pcr.id
