...
FROM  `compound_v2_ethereum`.`GovernorBravoDelegate_evt_ProposalCreated` pcr
LEFT JOIN cte_sum_votes csv ON csv.proposalId = pcr.id
LEFT JOIN `compound_v2_ethereum`.`GovernorBravoDelegate_evt_ProposalCanceled` pca ON pca.id = pcr.id
LEFT JOIN `compound_v2_ethereum`.`GovernorBravoDelegate_evt_ProposalExecuted` pex ON pex.id = pcr.id
LEFT JOIN `compound_v2_ethereum`.`GovernorBravoDelegate_evt_ProposalQueued` pqu ON pex.id = pcr.id
