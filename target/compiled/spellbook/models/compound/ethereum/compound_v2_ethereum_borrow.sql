...
with borrows as (
    ...
    from (
        select * from `compound_v2_ethereum`.`cErc20_evt_Borrow`
        
        union all
        select * from `compound_v2_ethereum`.`cEther_evt_Borrow`
...
repays as (
    select
        ...
        date_trunc('DAY', evt_block_time) as block_date
    from (
        select * from `compound_v2_ethereum`.`cErc20_evt_RepayBorrow`
        
        union all
        select * from `compound_v2_ethereum`.`cEther_evt_RepayBorrow`
...
liquidations as (
    select
        ...
        date_trunc('DAY', evt_block_time) as block_date
    from (
        select * from `compound_v2_ethereum`.`cErc20_evt_LiquidateBorrow`
        
        union all
        select * from `compound_v2_ethereum`.`cEther_evt_LiquidateBorrow`
