...
WITH
    trade_events as (
        ...
        FROM `archipelago_ethereum`.`ArchipelagoMarket_evt_Trade`
    ...
    token_events as (
        SELECT
            ...
            , tokenId as token_id
            , tradeId as unique_trade_id
        FROM `archipelago_ethereum`.`ArchipelagoMarket_evt_TokenTrade`
    ...
    fee_events as (
        SELECT
            ...
            , case when (
                ...
                as is_protocol_fee
        FROM `archipelago_ethereum`.`ArchipelagoMarket_evt_RoyaltyPayment`
...
left join `nft_ethereum`.`transfers` buyer_fix on buyer_fix.block_time=te.block_time
...
left join `nft_ethereum`.`transfers` seller_fix on seller_fix.block_time=te.block_time
