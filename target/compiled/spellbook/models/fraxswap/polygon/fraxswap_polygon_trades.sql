...
fraxswap_dex AS (
    SELECT
        ...
        ,''                                                                 AS trace_address
        ,t.evt_index
    FROM `fraxswap_polygon`.`FraxswapPair_evt_Swap` t
    INNER JOIN `fraxswap_polygon`.`FraxswapFactory_evt_PairCreated` p
        ON t.contract_address = p.pair
    
    
    WHERE t.evt_block_time >= '2022-05-22'
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2022-05-22'
