...
FROM `ironbank_optimism`.`CErc20Delegator_evt_Mint` m
LEFT JOIN `ironbank_optimism`.`itokens` i ON m.contract_address = i.contract_address
LEFT JOIN `prices`.`usd` p ON p.minute = date_trunc('minute', m.evt_block_time) AND p.contract_address = i.underlying_token_address AND p.blockchain = 'optimism'