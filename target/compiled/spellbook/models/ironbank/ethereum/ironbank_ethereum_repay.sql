...
FROM `ironbank_ethereum`.`CErc20Delegator_evt_RepayBorrow` r
LEFT JOIN `ironbank_ethereum`.`itokens` i ON i.contract_address = r.contract_address
LEFT JOIN `prices`.`usd` p ON p.minute = date_trunc('minute', r.evt_block_time) AND p.contract_address = i.underlying_token_address AND p.blockchain = 'ethereum'