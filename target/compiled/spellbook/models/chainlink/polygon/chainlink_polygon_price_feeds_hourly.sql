...
WITH gs AS (
    ...
    FROM (
        SELECT explode(
                ...
                proxy_address,
                aggregator_address
        FROM `chainlink_polygon`.`oracle_addresses`
    ) oa LEFT JOIN `chainlink_polygon`.`oracle_token_mapping` c ON c.proxy_address = oa.proxy_address
)
...
FROM (
    ...
    (
        ...
        FROM (
            ...
            FROM gs LEFT JOIN `chainlink_polygon`.`price_feeds` f
