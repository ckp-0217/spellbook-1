...
WITH gs AS (
    ...
    FROM (
        SELECT explode(
                sequence(
                            ...
                            DATE_TRUNC('hour', cast('2020-08-29' as date)),
    ...
    ) oa LEFT JOIN `chainlink_bnb`.`oracle_token_mapping` c ON c.proxy_address = oa.proxy_address
)
...
FROM (
    ...
    (
        ...
        FROM (
            ...
            FROM gs LEFT JOIN `chainlink_bnb`.`price_feeds` f
