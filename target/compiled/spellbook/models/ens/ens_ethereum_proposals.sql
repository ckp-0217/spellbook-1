...
with cte_support as (SELECT 
        ...
        proposalId
FROM `ethereumnameservice_ethereum`.`ENSGovernor_evt_VoteCast`
...
SELECT DISTINCT
    ...
    'DAO: ENS' as dao_name,
    '0x323a76393544d5ecca80cd6ef2a560c6a395b7e3' as dao_address,
    proposer,
    ...
    description
FROM  `ethereumnameservice_ethereum`.`ENSGovernor_evt_ProposalCreated` pcr
LEFT JOIN cte_sum_votes csv ON csv.proposalId = pcr.proposalId
LEFT JOIN `ethereumnameservice_ethereum`.`ENSGovernor_evt_ProposalCanceled` pca ON pca.proposalId = pcr.proposalId
LEFT JOIN `ethereumnameservice_ethereum`.`ENSGovernor_evt_ProposalExecuted` pex ON pex.proposalId = pcr.proposalId
LEFT JOIN `ethereumnameservice_ethereum`.`ENSGovernor_evt_ProposalQueued` pqu ON pex.proposalId = pcr.proposalId
