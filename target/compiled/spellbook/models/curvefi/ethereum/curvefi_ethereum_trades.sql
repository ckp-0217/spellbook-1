...
(
    ...
    JOIN  `curvefi_ethereum`.`view_pools` p
    ...
    WHERE l.topic1 IN
        ...
        AND l.block_time >= '2017-09-27 00:00:00'
...
INNER JOIN `ethereum`.`transactions` tx
    ...
    AND tx.block_time >= '2017-09-27 00:00:00'
    
    
LEFT JOIN `tokens_ethereum`.`erc20` erc20a ON erc20a.contract_address = dexs.token_bought_address
LEFT JOIN `tokens_ethereum`.`erc20` erc20b ON erc20b.contract_address = dexs.token_sold_address
LEFT JOIN `prices`.`usd` p_bought ON p_bought.minute = date_trunc('minute', dexs.block_time)
    ...
    -- The date below is derrived from `select min(evt_block_time) from uniswap_ethereum.Factory_evt_NewExchange;`
    -- If dexs above is changed then this will also need to be changed.
    AND p_bought.minute >= '2017-09-27 00:00:00'
...
LEFT JOIN `prices`.`usd` p_sold ON p_sold.minute = date_trunc('minute', dexs.block_time)
    ...
    -- The date below is derrived from `select min(evt_block_time) from uniswap_ethereum.Factory_evt_NewExchange;`
    -- If dexs above is changed then this will also need to be changed.
    AND p_sold.minute >= '2017-09-27 00:00:00'
