...
WITH base_pools AS (
    ...
    FROM `curvefi_optimism`.`StableSwap_call_coins`
    WHERE call_success
    GROUP BY
...
, meta_pools AS (
    ...
    (
        SELECT
            ...
            , bp.token
            , mp.evt_block_number
        FROM `curvefi_optimism`.`PoolFactory_evt_MetaPoolDeployed` mp
    ...
    INNER JOIN `erc20_optimism`.`evt_transfer` et
        ...
        AND et.evt_block_time >= '2022-01-17'
