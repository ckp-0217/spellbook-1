...
SELECT
    ...
    FROM (
        ...
        SELECT
            ...
            bought_id, 
            sold_id
        FROM `curvefi_optimism`.`StableSwap_evt_TokenExchange` t
        ...
        SELECT
            ...
            bought_id, 
            sold_id
        FROM `curvefi_optimism`.`MetaPoolSwap_evt_TokenExchangeUnderlying` t
        ...
        INNER JOIN `curvefi_optimism`.`pools` ta
        ...
        INNER JOIN `curvefi_optimism`.`pools` tb
    ...
    LEFT JOIN `tokens_optimism`.`erc20` ea
        ON ea.contract_address = ta.token
    LEFT JOIN `tokens_optimism`.`erc20` eb
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2022-01-17'
