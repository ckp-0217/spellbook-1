...
WITH event_data as (
    SELECT
        ...
        '' AS trace_address,
        evt_index
    FROM  `clipper_ethereum`.`ClipperVerifiedCaravelExchange_evt_Swapped`
    WHERE 1=1
    
    AND evt_block_time >= '2021-08-05'
...
INNER JOIN `ethereum`.`transactions` tx
    ...
    AND tx.block_time >= '2021-08-05'
    
    
LEFT JOIN `tokens`.`erc20` t_bought
    ON t_bought.contract_address = e.token_bought_address
    AND t_bought.blockchain = 'ethereum'
LEFT JOIN `tokens`.`erc20` t_sold
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2021-08-05'
