...
kyberswap_dex AS (
    ...
    FROM `kyber_optimism`.`DMM_Pool_evt_Swap` t
    INNER JOIN `kyberswap_optimism`.`DMM_Factory_evt_PoolCreated` p
        ON t.contract_address = p.pool
    
    WHERE t.evt_block_time >= '2021-08-31'
    ...
    FROM `kyber_optimism`.`Elastic_Pool_evt_swap` t
    INNER JOIN `kyber_optimism`.`Elastic_Factory_evt_PoolCreated` p
        ON t.contract_address = p.pool
    ...
    FROM `kyber_optimism`.`AggregationRouterV3_evt_Swapped`
    WHERE
        
        evt_block_time >= '2021-08-31'
    ...
    FROM `kyber_optimism`.`MetaAggregationRouter_evt_Swapped`
    WHERE
    ...
    FROM `kyberswap_optimism`.`MetaAggregationRouter_evt_Swapped`
    WHERE
...
LEFT JOIN `prices`.`usd` p_bought
    ...
    AND p_bought.minute >= '2021-08-31'
