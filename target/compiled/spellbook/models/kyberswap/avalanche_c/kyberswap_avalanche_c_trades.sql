...
kyberswap_dex AS (
    SELECT
        ...
        ,''                                                                 AS trace_address
        ,t.evt_index
    FROM `kyber_avalanche_c`.`DMMPool_evt_Swap` t
    INNER JOIN `kyber_avalanche_c`.`DMMFactory_evt_PoolCreated` p
    ...
    SELECT
        ...
        ,''                                                                            AS trace_address
        ,t.evt_index
    FROM `kyber_avalanche_c`.`Elastic_Pool_evt_swap` t
    INNER JOIN `kyber_avalanche_c`.`Elastic_Factory_evt_PoolCreated` p
        ON t.contract_address = p.pool
    ...
    SELECT
        ...
        ,''                                                                AS trace_address
        ,evt_index
    FROM `kyber_avalanche_c`.`AggregationRouter_evt_Swapped`
    WHERE
    ...
    SELECT
        ...
        ,''                                                                AS trace_address
        ,evt_index
    FROM `kyber_avalanche_c`.`MetaAggregationRouter_evt_Swapped`
    WHERE
