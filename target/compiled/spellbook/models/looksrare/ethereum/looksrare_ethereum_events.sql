...
WITH looksrare_trades AS (
    ...
    FROM (
        ...
        , ta.evt_index
        , ta.strategy
        FROM `looksrare_ethereum`.`LooksRareExchange_evt_TakerAsk` ta
        ...
        , tb.evt_index
        , tb.strategy
        FROM `looksrare_ethereum`.`LooksRareExchange_evt_TakerBid` tb
...
, royalties AS (
    ...
    FROM `looksrare_ethereum`.`LooksRareExchange_evt_RoyaltyPayment`
...
, platform_fees AS (
    ...
    FROM `looksrare_ethereum`.`StrategyStandardSaleForFixedPrice_call_viewProtocolFee`
    UNION ALL
    SELECT distinct contract_address
    , output_0/100 AS fee_percentage
    FROM `looksrare_ethereum`.`StrategyAnyItemFromCollectionForFixedPrice_call_viewProtocolFee`
    UNION ALL
    SELECT distinct contract_address
    , output_0/100 AS fee_percentage
    FROM `looksrare_ethereum`.`StrategyPrivateSale_call_viewProtocolFee`
    UNION ALL
    SELECT distinct contract_address
    , output_0/100 AS fee_percentage
    FROM `looksrare_ethereum`.`StrategyStandardSaleForFixedPriceV1B_call_viewProtocolFee`
    UNION ALL
    SELECT distinct contract_address
    , output_0/100 AS fee_percentage
    FROM `looksrare_ethereum`.`StrategyAnyItemFromCollectionForFixedPriceV1B_call_viewProtocolFee`
    )
...
INNER JOIN `ethereum`.`transactions` et ON lr.block_time=et.block_time
    AND lr.tx_hash=et.hash
    
LEFT JOIN `nft_ethereum`.`aggregators` agg ON et.to=agg.contract_address
LEFT JOIN `tokens_ethereum`.`nft` tok ON lr.nft_contract_address=tok.contract_address
LEFT JOIN royalties roy ON roy.block_time=lr.block_time
    ...
    AND roy.id = lr.id
LEFT JOIN `nft_ethereum`.`transfers` buyer_fix ON lr.block_time=buyer_fix.block_time
...
LEFT JOIN `nft_ethereum`.`transfers` seller_fix ON lr.block_time=seller_fix.block_time
...
LEFT JOIN `erc721_ethereum`.`evt_transfer` standard ON lr.block_time=standard.evt_block_time
