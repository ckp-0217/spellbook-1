...
FROM (
    ...
    , (SELECT chain_id FROM `chain_info`.`chain_ids` WHERE lower(chain_name) = 'optimism') AS source_chain_id
    ,chainId AS destination_chain_id
    
    FROM `hop_protocol_optimism`.`L2_OptimismBridge_evt_TransferSent` ts 
    ...
    , (SELECT chain_id FROM `chain_info`.`chain_ids` WHERE lower(chain_name) = 'optimism') AS destination_chain_id
    
    FROM `hop_protocol_optimism`.`L2_OptimismBridge_evt_TransferFromL1Completed` tl
    ...
    , CASE
            WHEN arb.transferId IS NOT NULL THEN (SELECT chain_id FROM `chain_info`.`chain_ids` WHERE lower(chain_name) = 'arbitrum one')
            WHEN poly.transferId IS NOT NULL THEN (SELECT chain_id FROM `chain_info`.`chain_ids` WHERE lower(chain_name) = 'polygon mainnet')
            WHEN gno.transferId IS NOT NULL THEN (SELECT chain_id FROM `chain_info`.`chain_ids` WHERE lower(chain_name) = 'gnosis')
        ELSE NULL
        END
        AS source_chain_id
    , (SELECT chain_id FROM `chain_info`.`chain_ids` WHERE lower(chain_name) = 'optimism') AS destination_chain_id
    FROM `hop_protocol_optimism`.`L2_OptimismBridge_evt_WithdrawalBonded` wb
        LEFT JOIN `hop_protocol_arbitrum`.`L2_ArbitrumBridge_evt_TransferSent` arb
        ...
        LEFT JOIN `hop_protocol_polygon`.`L2_PolygonBridge_evt_TransferSent` poly
        ...
        LEFT JOIN `hop_protocol_gnosis`.`L2_xDaiBridge_evt_TransferSent` gno
