

-- set max number of levels to trace root contract

-- set column names to loop through


with base_level as (
  select 
    creator_address
    ,contract_factory
    ,contract_address
    ,created_time
    ,creation_tx_hash
    ,is_self_destruct
  from (
    select 
      ct.`from` as creator_address
      ,CAST(NULL AS string) as contract_factory
      ,ct.address as contract_address
      ,ct.block_time as created_time
      ,ct.tx_hash as creation_tx_hash
      ,coalesce(sd.contract_address is not NULL, false) as is_self_destruct
    from `optimism`.`creation_traces` as ct 
    left join `contracts_optimism`.`self_destruct_contracts` as sd 
      on ct.address = sd.contract_address
      and ct.tx_hash = sd.creation_tx_hash
      and ct.block_time = sd.created_time
      
    where 
      true
       -- line 55 incremental filter
  ) as x
  group by 1, 2, 3, 4, 5, 6
)
,tokens as (
  select 
    bl.contract_address
    ,t.symbol
  from base_level as bl 
  join `tokens_optimism`.`erc20` as t
    on bl.contract_address = t.contract_address
  group by 1, 2

  union all 

  select 
    bl.contract_address
    ,t.name as symbol
  from base_level as bl 
  join `tokens_optimism`.`nft` as t
    on bl.contract_address = t.contract_address
  group by 1, 2
)
-- starting from 0 
,level0 as (
    select
      0 as level 
      ,coalesce(u.creator_address, b.creator_address) as creator_address
      ,case
        when u.creator_address is NULL then NULL
        else b.creator_address
      end as contract_factory
      
      ,b.contract_address
      ,b.created_time
      ,b.creation_tx_hash
      ,b.is_self_destruct
    from base_level as b
    left join base_level as u
      on b.creator_address = u.contract_address
    
),level1 as (
    select
      1 as level 
      ,coalesce(u.creator_address, b.creator_address) as creator_address
      ,case
        when u.creator_address is NULL then b.contract_factory
        else b.creator_address
      end as contract_factory
      
      ,b.contract_address
      ,b.created_time
      ,b.creation_tx_hash
      ,b.is_self_destruct
    from level0 as b
    left join base_level as u
      on b.creator_address = u.contract_address
    
),level2 as (
    select
      2 as level 
      ,coalesce(u.creator_address, b.creator_address) as creator_address
      ,case
        when u.creator_address is NULL then b.contract_factory
        else b.creator_address
      end as contract_factory
      
      ,b.contract_address
      ,b.created_time
      ,b.creation_tx_hash
      ,b.is_self_destruct
    from level1 as b
    left join base_level as u
      on b.creator_address = u.contract_address
    
),level3 as (
    select
      3 as level 
      ,coalesce(u.creator_address, b.creator_address) as creator_address
      ,case
        when u.creator_address is NULL then b.contract_factory
        else b.creator_address
      end as contract_factory
      
      ,b.contract_address
      ,b.created_time
      ,b.creation_tx_hash
      ,b.is_self_destruct
    from level2 as b
    left join base_level as u
      on b.creator_address = u.contract_address
    
),level4 as (
    select
      4 as level 
      ,coalesce(u.creator_address, b.creator_address) as creator_address
      ,case
        when u.creator_address is NULL then b.contract_factory
        else b.creator_address
      end as contract_factory
      
      ,b.contract_address
      ,b.created_time
      ,b.creation_tx_hash
      ,b.is_self_destruct
    from level3 as b
    left join base_level as u
      on b.creator_address = u.contract_address
    
)

,creator_contracts as (
  select 
    f.creator_address
    ,f.contract_factory
    ,f.contract_address
    ,coalesce(cc.contract_project, ccf.contract_project) as contract_project 
    ,f.created_time
    ,f.is_self_destruct
    ,f.creation_tx_hash
  from level4 as f
  left join `contracts_optimism`.`contract_creator_address_list` as cc 
    on f.creator_address = cc.creator_address
  left join `contracts_optimism`.`contract_creator_address_list` as ccf
    on f.contract_factory = ccf.creator_address
  where f.contract_address is not null
 )
,combine as (
  select 
    cc.creator_address
    ,cc.contract_factory
    ,cc.contract_address
    ,coalesce(cc.contract_project, oc.namespace) as contract_project 
    ,oc.name as contract_name 
    ,cc.created_time
    ,coalesce(cc.is_self_destruct, false) as is_self_destruct
    ,'creator contracts' as source
    ,cc.creation_tx_hash
  from creator_contracts as cc 
  left join `optimism`.`contracts` as oc 
    on cc.contract_address = oc.address 

  union all
  -- ovm 1.0 contracts

  select 
    creator_address
    ,NULL as contract_factory
    ,contract_address
    ,contract_project
    ,contract_name
    ,to_timestamp(created_time) as created_time
    ,false as is_self_destruct
    ,'ovm1 contracts' as source
    ,NULL as creation_tx_hash
  from `ovm1_optimism`.`contracts` as c
  where 
    true
    
    group by 1, 2, 3, 4, 5, 6, 7, 8, 9

  union all 
  --synthetix genesis contracts

  select 
    NULL as creator_address
    ,NULL as contract_factory
    ,snx.contract_address
    ,'Synthetix' as contract_project
    ,contract_name
    ,to_timestamp('2021-07-06 00:00:00') as created_time
    ,false as is_self_destruct
    ,'synthetix contracts' as source
    ,cast(NULL as string) as creation_tx_hash
  from `ovm1_optimism`.`synthetix_genesis_contracts` as snx
  where 
    true
    
    group by 1, 2, 3, 4, 5, 6, 7, 8, 9

    union all 
  --uniswap pools from ovm1

  select 
    NULL as creator_address
    ,NULL as contract_factory
    ,lower(newaddress) as contract_address
    ,'Uniswap V3' as contract_project
    ,'Pair' as contract_name
    ,to_timestamp('2021-11-11 00:00:00') as created_time
    ,false as is_self_destruct
    ,'ovm1 uniswap pools' as source
    ,NULL as creation_tx_hash
  from `uniswap_v3_optimism`.`ovm1_pool_mapping` as uni
  where 
    true
    
    group by 1, 2, 3, 4, 5, 6, 7, 8, 9
)
,get_contracts as (
  select 
    c.contract_address
    ,c.contract_factory
    ,c.contract_project
    ,t.symbol as token_symbol
    ,c.contract_name
    ,c.creator_address
    ,c.created_time 
    ,c.is_self_destruct
    ,c.creation_tx_hash
  from combine as c 
  left join tokens as t 
    on c.contract_address = t.contract_address
  group by 1, 2, 3, 4, 5, 6, 7, 8, 9
)
,cleanup as (
--grab the first non-null value for each, i.e. if we have the contract via both contract mapping and optimism.contracts
  select 
    contract_address
    
    ,(array_agg(contract_project) filter (where contract_project is not NULL))[0] as contract_project
    
    ,(array_agg(token_symbol) filter (where token_symbol is not NULL))[0] as token_symbol
    
    ,(array_agg(contract_name) filter (where contract_name is not NULL))[0] as contract_name
    
    ,(array_agg(creator_address) filter (where creator_address is not NULL))[0] as creator_address
    
    ,(array_agg(created_time) filter (where created_time is not NULL))[0] as created_time
    
    ,(array_agg(contract_factory) filter (where contract_factory is not NULL))[0] as contract_factory
    
    ,(array_agg(is_self_destruct) filter (where is_self_destruct is not NULL))[0] as is_self_destruct
    
    ,(array_agg(creation_tx_hash) filter (where creation_tx_hash is not NULL))[0] as creation_tx_hash
    
  from get_contracts
  where contract_address is not NULL 
  group by 1
)
select 
  c.contract_address
  ,initcap(
      replace(
      -- priority order: Override name, Mapped vs Dune, Raw/Actual names
        coalesce(
          co.contract_project
          ,dnm.mapped_name
          ,c.contract_project
          ,ovm1c.contract_project
        ),
      '_',
      ' '
    )
   ) as contract_project
  ,c.token_symbol
  ,coalesce(co.contract_name, c.contract_name) as contract_name
  ,coalesce(c.creator_address, ovm1c.creator_address) as creator_address
  ,coalesce(c.created_time, to_timestamp(ovm1c.created_time)) as created_time
  ,coalesce(c.contract_factory, 
  cast(NULL as string)
  
  ) as contract_creator_if_factory
  ,coalesce(c.is_self_destruct, false) as is_self_destruct
  ,c.creation_tx_hash
from cleanup as c 
left join `ovm1_optimism`.`contracts` as ovm1c
  on c.contract_address = ovm1c.contract_address --fill in any missing contract creators
left join `contracts_optimism`.`project_name_mappings` as dnm -- fix names for decoded contracts
  on lower(c.contract_project) = lower(dnm.dune_name)
left join `contracts_optimism`.`contract_overrides` as co --override contract maps
  on lower(c.contract_address) = lower(co.contract_address)
