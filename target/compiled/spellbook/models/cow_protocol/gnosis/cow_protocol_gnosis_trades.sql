...
trades_with_prices AS (
    ...
    FROM `gnosis_protocol_v2_gnosis`.`GPv2Settlement_evt_Trade` settlement
...
trades_with_token_units as (
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN `tokens_gnosis`.`erc20` ts
                             ON ts.contract_address = sell_token
             LEFT OUTER JOIN `tokens_gnosis`.`erc20` tb
...
order_ids as (
    ...
    from (  select orderUid, evt_tx_hash, evt_index
            from `gnosis_protocol_v2_gnosis`.`GPv2Settlement_evt_Trade`
