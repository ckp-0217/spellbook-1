...
batch_counts as (
    ...
    from `gnosis_protocol_v2_gnosis`.`GPv2Settlement_evt_Settlement` s
        left outer join `gnosis_protocol_v2_gnosis`.`GPv2Settlement_evt_Interaction` i
...
combined_batch_info as (
    ...
    from batch_counts b
        ...
        inner join `gnosis`.`transactions` tx
            on evt_tx_hash = hash
