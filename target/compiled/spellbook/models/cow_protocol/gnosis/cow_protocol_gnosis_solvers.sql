...
solver_activation_events as (
    ...
    from `gnosis_protocol_v2_gnosis`.`GPv2AllowListAuthentication_evt_SolverAdded`
    union
    select solver, evt_block_number, evt_index, False as activated
    from `gnosis_protocol_v2_gnosis`.`GPv2AllowListAuthentication_evt_SolverRemoved`
),
