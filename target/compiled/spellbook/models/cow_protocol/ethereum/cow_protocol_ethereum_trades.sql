...
trades_with_prices AS (
    ...
    FROM `gnosis_protocol_v2_ethereum`.`GPv2Settlement_evt_Trade` trade
             LEFT OUTER JOIN `prices`.`usd` as ps
                             ON sellToken = ps.contract_address
                                 AND ps.minute = date_trunc('minute', evt_block_time)
                                 AND ps.blockchain = 'ethereum'
                                 
             LEFT OUTER JOIN `prices`.`usd` as pb
...
trades_with_token_units as (
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN `tokens_ethereum`.`erc20` ts
                             ON ts.contract_address = sell_token
             LEFT OUTER JOIN `tokens_ethereum`.`erc20` tb
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join `gnosis_protocol_v2_ethereum`.`GPv2Settlement_call_settle`
...
eth_flow_senders as (
    ...
    from `cow_protocol_ethereum`.`CoWSwapEthFlow_evt_OrderPlacement` event
    inner join `cow_protocol_ethereum`.`CoWSwapEthFlow_call_createOrder` call
